    ...
    async def get_initial_chart_data(
        ...
        try:
            ...
            for candle in raw_data:
                formatted_candle = {
                    'timestamp': int(candle[0]),  # Keep milliseconds for backend consistency
                    'open': float(candle[1]),
                    'high': float(candle[2]),
                    'low': float(candle[3]),
            ...
            logger.info(f"Successfully formatted {len(formatted_data)} candles for {symbol} {timeframe}")
            ...
            logger.error(f"Error fetching initial chart data for {symbol} {timeframe}: {str(e)}")
            raise Exception(f"Failed to fetch chart data: {str(e)}")
    
    async def format_realtime_update(self, candle_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def validate_candle_data(self, candle_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not (low_price <= open_price <= high_price and 
                ...
                logger.warning(f"Invalid OHLC relationships: O={open_price}, H={high_price}, L={low_price}, C={close_price}")
                return False
    ...
    async def prepare_websocket_message(
        ...
        try:
            if not raw_candle or len(raw_candle) < 6:
                logger.warning(f"Invalid raw candle data for {symbol}: {raw_candle}")
                return None
            ...
            logger.error(f"Error preparing WebSocket message for {symbol}: {e}")
            return None
