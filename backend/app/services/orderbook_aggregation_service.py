...
class OrderBookAggregationService:
    ...
    def get_exact_levels(
            self,
            raw_data: List[Dict],
            is_ask: bool,
            effective_depth: int,
            effective_rounding: float) -> List[Dict]:
        """
        ...
        if effective_rounding >= 1.0:
            logger.debug(
                f"Aggregating {
                    'asks' if is_ask else 'bids'} with rounding={effective_rounding}, raw_data_count={
                    len(raw_data)}")
        ...
        for item in raw_data:
            ...
            rounded_price = (
                self.round_up(
                    price,
                    effective_rounding) if is_ask else self.round_down(
                    price,
                    effective_rounding))
            ...
            if effective_rounding >= 1.0 and len(buckets) < 5:
                logger.debug(
                    f"Price {price} -> rounded to {rounded_price} (rounding={effective_rounding})")
        ...
        if effective_rounding >= 1.0:
            logger.debug(
                f"Buckets after aggregation: {
                    list(
                        buckets.keys())[
                        :10]}")  # Show first 10 prices
        ...
        for price, amount in buckets.items():
            # Use a more aggressive threshold to catch floating point precision
            # issues
        ...
        # Now take exactly effective_depth levels - all are guaranteed to be
        # non-zero
        result_levels = levels[:effective_depth]

        # Final verification - ensure no zeros slipped through due to floating
        # point issues
        result_levels = [
            level for level in result_levels if level['amount'] > 1e-6]

        # Debug logging final result only when there's an issue
        if len(result_levels) < effective_depth:
            # Use debug level for empty orderbooks (0 levels), warning for
            # partial data
            if len(result_levels) == 0:
                logger.debug(
                    f"[ORDERBOOK_AGGREGATION] Empty {
                        'asks' if is_ask else 'bids'}: requested={effective_depth}, rounding={effective_rounding}")
            else:
                logger.warning(
                    f"[ORDERBOOK_AGGREGATION] Insufficient {
                        'asks' if is_ask else 'bids'} levels: requested={effective_depth}, got={
                        len(result_levels)}, " f"rounding={effective_rounding}")
    ...
    def calculate_cumulative_totals(
        ...
        if is_ask:
            # For asks (highest price first), cumulative total is from current
            # level to end (all better prices)
            for i, level in enumerate(levels):
                cumulative_total = sum(level_item['amount'] for level_item in levels[i:])
                result.append({
                    'price': level['price'],
                    'amount': level['amount'],
            ...
            for i, level in enumerate(levels):
                cumulative_total = sum(level_item['amount'] for level_item in levels[:i + 1])
                result.append({
                    'price': level['price'],
                    'amount': level['amount'],
                    'cumulative': cumulative_total
    ...
    def analyze_market_depth(
        ...
        has_insufficient_data = (len(raw_bids) < min_required_raw_data
                                 or len(raw_asks) < min_required_raw_data)

        # Get aggregated levels to check actual available levels
        aggregated_bids = self.get_exact_levels(
            raw_bids, False, effective_depth, effective_rounding)
        aggregated_asks = self.get_exact_levels(
            raw_asks, True, effective_depth, effective_rounding)
    ...
    async def aggregate_orderbook(
            ...
            limit: int,
            rounding: float,
            symbol_data: Optional[Dict] = None) -> Dict:
        """
        ...
        cache_key = self._generate_cache_key(
            orderbook.symbol, limit, rounding, orderbook.timestamp)
        ...
        while attempt < max_attempts:
            ...
            aggregated_bids = self.get_exact_levels(
                raw_bids, False, limit, rounding)
            aggregated_asks = self.get_exact_levels(
                raw_asks, True, limit, rounding)
            ...
            if len(aggregated_bids) >= limit and len(aggregated_asks) >= limit:
                ...
                logger.debug(
                    f"[ORDERBOOK_AGGREGATION] Fetching more data for {
                        orderbook.symbol}: " f"current levels: bids={
                        len(aggregated_bids)}, asks={
                        len(aggregated_asks)}, requested={limit}")
                multiplier *= 2
                attempt += 1

        # If we still don't have enough levels after max attempts, log
        # appropriately
        if len(aggregated_bids) < limit or len(aggregated_asks) < limit:
            # Use debug for completely empty orderbooks (initial load), warning
            # for partial data
            if len(aggregated_bids) == 0 and len(aggregated_asks) == 0:
                logger.debug(
                    f"[ORDERBOOK_AGGREGATION] Empty orderbook for {
                        orderbook.symbol} after {max_attempts} attempts")
            else:
                logger.warning(
                    f"[ORDERBOOK_AGGREGATION] Could not get {limit} levels for {
                        orderbook.symbol} after {max_attempts} attempts. " f"Got bids={
                        len(aggregated_bids)}, asks={
                        len(aggregated_asks)}")

        # Analyze market depth
        market_depth_info = self.analyze_market_depth(
            raw_bids, raw_asks, limit, rounding)

        # Calculate cumulative totals for bids (highest to lowest)
        bids_with_cumulative = self.calculate_cumulative_totals(
            aggregated_bids, False)

        # For asks: reverse order first (highest price at top), then calculate
        # cumulative
        aggregated_asks.reverse()  # Now highest price first
        asks_with_cumulative = self.calculate_cumulative_totals(
            aggregated_asks, True)

        # Apply formatting to all levels if symbol_data is available
        if symbol_data:
            # Format bids
            for bid in bids_with_cumulative:
                bid['price_formatted'] = formatting_service.format_price(
                    bid['price'], symbol_data, rounding)
                bid['amount_formatted'] = formatting_service.format_amount(
                    bid['amount'], symbol_data)
                bid['cumulative_formatted'] = formatting_service.format_total(
                    bid['cumulative'], symbol_data)

            # Format asks
            for ask in asks_with_cumulative:
                ask['price_formatted'] = formatting_service.format_price(
                    ask['price'], symbol_data, rounding)
                ask['amount_formatted'] = formatting_service.format_amount(
                    ask['amount'], symbol_data)
        ...
        zero_bids_in_result = [
            b for b in result['bids'] if b['amount'] <= 1e-6]
        zero_asks_in_result = [
            a for a in result['asks'] if a['amount'] <= 1e-6]
        if zero_bids_in_result or zero_asks_in_result:
            logger.error(
                f"[ZERO_CHECK] CRITICAL: Zeros found in final result for {
                    orderbook.symbol}! " f"Zero bids: {zero_bids_in_result}, Zero asks: {zero_asks_in_result}")
    ...
    async def get_cache_metrics(self) -> Dict:
        ...
        async with self._cache_lock:
            hit_rate = (
                ...
                * 100) if self._total_requests > 0 else 0
            return {
    ...
    async def warm_cache_for_symbol(
        ...
        try:
            ...
            # If orderbook is empty or has issues, current_price will remain
            # None
            pass
        ...
        for limit in common_limits:
            for rounding in common_roundings:
                try:
                    ...
                    logger.warning(
                        f"Cache warming failed for {symbol} limit={limit} rounding={rounding}: {e}")
                    continue
