...
// Enhanced error logging function for better monitoring
function logError(context, error, additionalData = {}) {
  ...
  console.error(`[OrderFox Frontend Error] ${context}:`, errorInfo);
...
function logWebSocketEvent(event, symbol, type, additionalData = {}) {
  const logInfo = {
    timestamp: new Date().toISOString(),
    event,
    symbol,
  ...
  console.log(`[OrderFox WebSocket] ${event}:`, logInfo);
  
  // In production, you could send WebSocket events to monitoring
  // sendToMonitoring('websocket', logInfo);
  
  return logInfo;
}

// Helper function to handle API responses consistently with enhanced logging
function handleApiResponse(response, data, defaultErrorMessage, context = 'API Request') {
  if (!response.ok) {
    const errorMessage = data.detail || data.message || data.error || defaultErrorMessage;
  ...
  // Check for PHP backend error response
  if (data.success === false) {
    const errorMessage = data.message || data.error || 'API request failed';
    logError(context, new Error(errorMessage), {
      phpBackendError: true,
      responseData: data
    });
    throw new Error(errorMessage);
  }
...
async function fetchSymbols() {
  ...
  notify('symbolsError');
  try {
    const response = await fetch(`${API_BASE_URL}/market-data/symbols`);
    const data = await response.json();
    handleApiResponse(response, data, 'Failed to fetch symbols', 'Fetch Symbols');
    
    const symbols = data.data?.symbols || data.symbols || data;
    state.symbolsList = symbols.map(symbol => ({
      ...symbol,
    ...
    console.log(`[OrderFox] Successfully fetched ${symbols.length} symbols`);
  } catch (error) {
    logError('Fetch Symbols', error, { symbolsCount: state.symbolsList.length });
    state.symbolsLoading = false;
...
async function fetchOrderBook(symbol, limit) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/market-data/orderbook/${symbol}${params}`);
    const data = await response.json();
    handleApiResponse(response, data, 'Failed to fetch order book');
    
    const orderbook = data.data?.orderbook || data.orderbook || data;
    ...
    logError('Fetch OrderBook', error, { 
      symbol, 
      limit, 
      validLimit,
      currentOrderBookSymbol: state.currentOrderBook?.symbol 
    });
    state.orderBookLoading = false;
...
async function fetchCandles(symbol, timeframe = '1m', limit = 100) {
  ...
  notify('candlesError');
  try {
    const response = await fetch(`${API_BASE_URL}/market-data/candles/${symbol}/${timeframe}?limit=${limit}`);
    const data = await response.json();
    handleApiResponse(response, data, 'Failed to fetch candles');
    
    const candles = data.data?.candles || data.candles || data;
    state.currentCandles = validateAndFilterCandles(candles);
    state.candlesLoading = false;
    notify('currentCandles');
    notify('candlesLoading');
  } catch (error) {
    logError('Fetch Candles', error, { 
      symbol, 
      timeframe, 
      limit,
      currentCandlesCount: state.currentCandles.length 
    });
    state.candlesLoading = false;
...
// Trading functionality removed - not part of PHP backend migration
// These functions were for the FastAPI backend trading features
// Market data migration complete, trading features excluded per migration scope

async function fetchOpenPositions() {
  // Trading functionality not implemented in PHP backend migration
  console.warn('Trading functionality not available in PHP backend migration');
...
async function executeTrade(tradeDetails, mode) {
  // Trading functionality not implemented in PHP backend migration
  console.warn('Trading functionality not available in PHP backend migration');
  return Promise.reject(new Error('Trading functionality not available in PHP backend migration'));
}

async function executePaperTrade(tradeDetails) {
  return executeTrade(tradeDetails, 'paper');
}

async function executeLiveTrade(tradeDetails) {
  return executeTrade(tradeDetails, 'live');
}

async function setTradingModeApi(mode) {
  // Trading functionality not implemented in PHP backend migration
  console.warn('Trading functionality not available in PHP backend migration');
  state.tradingMode = mode; // Update state locally only
  notify('tradingMode');
}
