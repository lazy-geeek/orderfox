...
// Enhanced WebSocket logging function
...
// WebSocket error logging function
function logWebSocketError(context, error, symbol, type, additionalData = {}) {
  const errorInfo = {
    timestamp: new Date().toISOString(),
    context,
  ...
  console.error(`[OrderFox WebSocket Error] ${context}:`, errorInfo);
  return errorInfo;
}
...
export const connectWebSocketStream = async (
  ...
  if (connectionInProgress[streamKey]) {
    logWebSocketEvent('Connection Skipped - In Progress', symbol, streamType, { streamKey, timeframe, limit });
    return;
  }
  ...
  if (activeWebSockets[streamKey]) {
    ...
    if (currentState === WebSocket.OPEN) {
      logWebSocketEvent('Connection Skipped - Already Open', symbol, streamType, { streamKey, readyState: currentState });
      return;
    } else if (currentState === WebSocket.CONNECTING) {
      logWebSocketEvent('Connection Skipped - Already Connecting', symbol, streamType, { streamKey, readyState: currentState });
      return;
    }
  }
  ...
  try {
    ...
    ws.onopen = () => {
      logWebSocketEvent('Connection Opened', symbol, streamType, { 
        streamKey, 
        wsUrl, 
        timeframe, 
        limit,
        attemptCount: connectionAttempts[streamKey] || 0 
      });
    ...
    ws.onmessage = (event) => {
      try {
        ...
        logWebSocketError('Message Parse Error', error, symbol, streamType, { 
          streamKey, 
          messageData: event.data?.substring(0, 200), // First 200 chars for debugging
    ...
    ws.onerror = (error) => {
      ...
      logWebSocketError('Connection Error', error, symbol, streamType, { 
        streamKey, 
        attemptCount, 
        wsUrl,
